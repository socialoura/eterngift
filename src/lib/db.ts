...
export async function initDatabase() {
  try {
    ...
    await sql`
      INSERT INTO products (id, name, description, image_url, base_price, stock)
      VALUES ('eternal-rose-box', 'Eternal Rose Box with Engraved Necklace', 'Eternal luxury in a box. A stunning preserved rose arrangement paired with a personalized engraved necklace.', '/products/rose-box/rouge/1.png', 129.99, 100)
      ON CONFLICT (id) DO NOTHING
    `
    await sql`
      INSERT INTO products (id, name, description, image_url, base_price, stock)
      VALUES ('eternal-rose-bear', 'Eternal Rose Bear with Engraved Necklace', 'Your perfect eternal companion. A stunning rose bear paired with an engraved necklace, creating a timeless symbol of love and remembrance.', '/products/rose-bear/rouge/1.png', 89.99, 100)
      ON CONFLICT (id) DO NOTHING
    `
